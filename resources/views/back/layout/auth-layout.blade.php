...
<html>
	<head>
		<!-- Basic Page Info -->
		<meta charset="utf-8" />
		<title>@yield('BoardingHouse')</title>
	...
	<body class="login-page">
        ...
        <script>
			 ...
			 //		toastr.remove();
			 //	  if( event[0].type === 'info' ){ toastr.info(event.detail[0].message); }
			 //	  else if( event[0].type === 'success' ){ toastr.success(event.detail[0].message); }
				//  else if( event[0].type === 'error' ){ toastr.error(event.detail[0].message); }
			 //	  else if( event[0].type === 'warning' ){ toastr.warning(event.detail[0].message); }
			 //	  else{ return false; }
			 //	 });
			// });
		</script>
